...
<div>
  <w-form>
  <w-input v-model="inputip" label="IP Address" :validators="[validators.ip]"></w-input>
  <div class="text-right mt3">
    <w-button :disabled="loading" @click="getIp(inputip)" type="submit" >Submit</w-button>
  </div>
  </w-form>
  <div v-if="loading">
    <p>Loading AWS IPs</p>
   <w-progress class="ma1" circle="circle" :value="progress" size="5em"><strong>{{ progress }}%</strong></w-progress>
  </div>
        ...
        .then((response) => {
          if (!response.ok) {
            ...
            response.json().then((data) => {
              this.cfips = this.filterCFips(data);
            });
          }
        })
